...
namespace MettleSystems.dashCommerce.Controls {
  ...
  [ToolboxData("<{0}:RequiredFieldValidator ID=\"RequiredFieldValidatorId\" runat=\"server\" />")]
  public class RequiredFieldValidator : System.Web.UI.WebControls.RequiredFieldValidator {

    protected override void Render(HtmlTextWriter writer) {
      if (!string.IsNullOrEmpty(this.ID) && this.ErrorMessage.Length == 0) {
        this.ErrorMessage = LocalizationUtility.GetText(this.ID);
      }
      base.Render(writer);
    }
